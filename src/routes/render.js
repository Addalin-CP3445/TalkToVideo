...
`;
      ...
      // If the LLM stopped generating scenes early, we stretch the final scene 
      // to cover the remaining time so the video doesn't cut off early.
      let runningSum = 0;
      ...
      // Drawtext captions
      const captionFilters = buildCaptionFilter(segments, themeKey, WIDTH, HEIGHT, tempFiles);
      ...
      if (theme.bgGradient) {
        bgFilter =
          ...
          `[top][bot]vstack=inputs=2[bg];[bg]${buildCaptionFilter(segments, themeKey, WIDTH, HEIGHT, tempFiles)}[vout]`;
      } else {
        bgFilter = `color=c=${theme.bgColor}:size=${WIDTH}x${HEIGHT}:d=${totalDuration},format=rgb24[bg];[bg]${buildCaptionFilter(segments, themeKey, WIDTH, HEIGHT, tempFiles)}[vout]`;
      }
    }
