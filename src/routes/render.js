...
`;
      ...
      const filterSegments = downloadedScenes.map((ds, idx) => {
        ...
        if (ds.type === 'video') {
          ...
          // Scale to fill 1920x1080 (zoom/crop), FORCE framerate to 30fps to prevent concat drift, trim to duration, offset to exact start time
          return `[${inputIdx}:v]scale=1920:1080:force_original_aspect_ratio=increase,crop=1920:1080,fps=30,trim=duration=${ds.duration.toFixed(2)},setpts=PTS-STARTPTS+${ds.start.toFixed(2)}/TB${outStream}`;
        } else {
          // Slide
          ...
          return `${colorFilter},drawtext=textfile='${safePath}':fontcolor=${fontColor}:fontsize=90:x=(w-text_w)/2:y=(h-text_h)/2:shadowcolor=${shadowColor}:shadowx=4:shadowy=4:box=1:boxcolor=0x00000088:boxborderw=20,setpts=PTS-STARTPTS+${ds.start.toFixed(2)}/TB${outStream}`;
        }
      });

      inputFiles = downloadedScenes.filter(ds => ds.type === 'video').map(ds => ds.localPath);
      
      const baseColor = theme.bgColor || '0x0f1625';
      const baseFilter = `color=c=${baseColor}:size=1920x1080:d=${totalDuration}:r=30[base]`;
      
      let overlayChain = filterSegments.length > 0 ? '' : '[base]null[bg]';
      let prevStream = '[base]';
      
      downloadedScenes.forEach((ds, idx) => {
        const nextStream = idx === downloadedScenes.length - 1 ? '[bg]' : `[bg${idx}]`;
        overlayChain += `;${prevStream}[v${idx}]overlay=eof_action=pass:enable='between(t,${ds.start.toFixed(2)},${ds.end.toFixed(2)})'${nextStream}`;
      ...
      // Full filter chain: processing individual inputs -> overlay them -> drawtext -> output
      bgFilter = `${baseFilter};${filterSegments.join(';')}${overlayChain};[bg]${captionFilters ? captionFilters + '[vout]' : 'null[vout]'}`;
